...
def abrir_progress():
    # Código para crear una nueva ventana
    pkey = tk.Tk()
    nueva_ventana.title("Progress Bar")
    # Añade los widgets y la lógica de la nueva ventana aquí

def cerrar_ventana_actual():
    # Código para cerrar la ventana actual
    root.destroy()
    # Llama a la función para abrir la nueva ventana
    abrir_keyboard()

def actualizar_barra_progreso():
    for i in range(21):
        barra_progreso["value"] = i * 5
        time.sleep(1)
def abrir_keyboard():
    key = tk.Tk()  # key window name
    key.title('Keyboard By MrCrow') 
    ...
    screen_width = key.winfo_screenwidth()
    screen_height = key.winfo_screenheight()

    # Size window size
    key.geometry(f"{screen_width}x{screen_height}")
    key.maxsize(width=screen_width, height=screen_height)
    key.minsize(width=screen_width, height=screen_height)
    # end window size
...
cerrar_btn = tk.Button(root, text="Cerrar Ventana", command=cerrar_ventana_actual)
cerrar_btn.pack()

# Iniciar la actualización de la barra de progreso en un hilo separado
hilo_barra_progreso = Thread(target=actualizar_barra_progreso)
hilo_barra_progreso.start()

# El resto de widgets y la lógica de la ventana principal
...
'''
key = tk.Tk()  # key window name
key.title('Keyboard By MrCrow')  # title Name
